...
class Group:
    ...
    def join(self, user, room_name=None):
        self.members[user] = S_ALONE

        if room_name:
            if room_name not in self.chat_grps:
                self.chat_grps[room_name] = []
            if user not in self.chat_grps[room_name]:
                self.chat_grps[room_name].append(user)
            return
    ...
    def disconnect(self, me):
        ...
        if in_group == True:
            ...
            if len(self.chat_grps[group_key]) == 0:
